...
async def process_pending(creds: LlmCredentials, settings: Settings, repository: EvaluationRepository) -> list[str]:
    ...
    for file in files:
        with logfire.span('evaluation.process_file', file_id=file.file_id):
            try:
            ...
            except Exception as exc:  # noqa: BLE001 — skip this file, keep the batch going
                ...
                except Exception:  # noqa: BLE001 — best-effort; must not break the rest of the batch
...
async def retry_file(
    ...
    """
    Manually retry section detection for one specific file — same detect+store+notify logic
    as `process_pending`'s per-file body, scoped to a single file for the retry endpoint.
    ...
    await repository.set_detection_error(file_id, None)
    try:
        result = await _detect(creds, settings, file)
        evaluation_id = await repository.create_evaluation(file, result.technical, result.price, result.model)
    except Exception as exc:  # noqa: BLE001 — record and let the caller see it via detection_error
        logfire.exception('evaluation.retry_file failed', file_id=file_id)
        await repository.set_detection_error(file_id, str(exc))
        return None
    if evaluation_id is None:
        return None
