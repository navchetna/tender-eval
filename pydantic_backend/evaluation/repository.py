...
class EvaluationRepository:
    ...
    async def get_file_for_retry(self, file_id: str) -> PendingFile | None:
        """The PARSED file_repository row for this doc_type, if it still has no evaluation —
        same eligibility predicate as `claim_pending_files` but scoped to one file_id, for the
        manual retry endpoint (e.g. after a previous detection attempt failed)."""
        async with pool_connection(self.settings) as connection:
            async with connection.cursor() as cursor:
                await cursor.execute(
                    f'''
                    ...
                    FROM file_repository f
                    LEFT JOIN {self._table} e ON e.file_id = f.file_id
                    WHERE f.file_id = %s AND f.file_type = %s
                      AND f.processing_status = 'PARSED' AND e.evaluation_id IS NULL
                    ''',
                    (file_id, self.doc_type.value),
                )
                row = await cursor.fetchone()
        if row is None:
            return None
        return PendingFile(**{**row, 'file_id': str(row['file_id']), 'project_id': str(row['project_id'])})
            ...
            async with connection.cursor() as cursor:
                await cursor.execute(
                    ...
                    INSERT INTO {self._table} (
                        ...
                        technical_ai_title, technical_ai_content,
                        price_section_title, price_section_content,
                        price_ai_title, price_ai_content
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (file_id) DO NOTHING
                    ''',
                    (
                        evaluation_id, file.file_id, file.project_id, file.version, model,
                        technical.heading, technical.content, technical.heading, technical.content,
                        price.heading, price.content, price.heading, price.content,
                    ),
                )
    ...
    async def set_detection_error(self, file_id: str, error: str | None) -> None:
        """Record (or clear) the most recent detection-stage failure for this file, so a
        permanently-failing file is distinguishable from one that just hasn't been attempted yet."""
        async with pool_connection(self.settings) as connection:
            async with connection.cursor() as cursor:
                await cursor.execute(
                    'UPDATE file_repository SET detection_error = %s, updated_at = now() WHERE file_id = %s',
                    (error, file_id),
                )
            await connection.commit()
    ...
    async def get_evaluation_by_file(self, file_id: str) -> EvaluationRecord | None:
        async with pool_connection(self.settings) as connection:
            async with connection.cursor() as cursor:
                await cursor.execute(
                    f'''
...
def _to_record(row: dict) -> EvaluationRecord:
    return EvaluationRecord(
        ...
        technical_ai_title=row['technical_ai_title'],
        technical_ai_content=row['technical_ai_content'],
        ...
        price_ai_title=row['price_ai_title'],
        price_ai_content=row['price_ai_content'],
