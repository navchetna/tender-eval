...
export function DetailTabs({
  ...
  return (
    <Card className="expand mt-4 overflow-hidden">
      ...
      <div className={`bg-surface p-4 ${tab === "doc" || tab === "tree" ? "max-h-[90vh] overflow-auto" : ""}`}>
        {/* Kept mounted (hidden, not unmounted) across tab switches — PdfViewer fetches the PDF
            in an effect keyed on [projectId, fileId], so unmounting/remounting it on every tab
            switch re-downloaded the whole file each time even though nothing changed. */}
        <div className={tab === "doc" ? "" : "hidden"}>
          <PdfViewer projectId={file.project_id} fileId={file.file_id} />
        </div>
        {tab === "tree" &&
          (tree ? (
            <ParsedTreeView tree={tree} projectId={file.project_id} fileId={file.file_id} />
          ) : (
